...
class Program
{
    ...
    static void HelpMenu()
    {
        ...
        PrintFunctionDescription("delete <serverPath>", "Delete a file or folder at the specified path on the server.");
        ...
        PrintFunctionUsage(@"send <serverPath> <localPath>", @"send c:\ and type a local folder c:\Program Files\My Program");
        ...
        PrintFunctionUsage(@"proc <type(kill)> <processID>", @"proc kill 1234 >> 1234 is a process ID");
    }
    ...
    static void ReceiveFileOrDirectory(NetworkStream stream, string currentDirectory = "")
    {
        while (true)
        {
            ...
            if (message.StartsWith("Directory"))
            {
                ...
                ReceiveFileOrDirectory(stream, directoryPath); // Recursively process subfolders
            }
            else if (message.StartsWith("End of directory"))
            {
                Console.WriteLine(message);
                EndProcessMessage(stream, "End of directory ok");
                return; // Return to previous folder after finishing current directory
            }
            else
            {
    ...
    static void ReceiveFile(NetworkStream stream, string defaultFileName)
    {
        ...
        // Send confirmation to server that the file was successfully received
    ...
    static void SendDirectory(string directoryPath, NetworkStream stream)
    {
        ...
        // Notify the start of directory content transmission

        // Send files in the directory
        ...
        // Notify the end of directory transmission
        SendResponse(stream, "End of directory");
    ...
    static bool EndProcessMessageGeter(NetworkStream stream, string processName)
    {
        ...
        // NEED TO FINISH THIS FUNCTION TO RECEIVE CONFIRMATION FROM CLIENT!!!!!!!!!!!!!!!!!
    }
    ...
    static void SendFile(string filePath, NetworkStream stream)
    {
        ...
        // Wait for confirmation from client that the file name was received
